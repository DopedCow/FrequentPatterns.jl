...
player = Actor("player.png", (400, 550))
boss = Actor("boss.png")
gameStatus = 0
highScore = []
...
def drawHighScore():
    ...
    for line in highScore:
        if y < 400:
            ...
            y += 50
    screen.draw.text("Press Escape to play again", center = (400, 550), owidth = 0.5, ocolor = (255, 255, 255), color = (266, 64, 0), fontsize = 60)


def drawLives():
    for l in range(player.lives): screen.blit("life.png", (10+(l*32), 10))
...
def checkKeys():
    ...
    if keyboard.space:
        if player.laserActive == 1:
            ...
            lasers.append(Actor("laser2.png", (player.x, player.y-32)))
...
def updateAliens():
    ...
    for a in range(len(aliens)):
        ...
        if randint(0, 1) == 0:
            aliens[a].image = "alien1.png"
        else:
            aliens[a].image = "alien1b.png"
            if randint(0, 5) == 0:
                lasers.append(Actor("laser1.png", (aliens[a].x, aliens[a].y)))
                lasers[len(lasers)-1].status = 0
...
def init():
    ...
    moveDelay = 30
    boss.active = False
    player.images = ["player.png", "explosion1.png", "explosion2.png", "explosion3.png", "explosion4.png", "explosion5.png"]
    player.laserActive = 1
    player.lives = 3
    player.name = ""
    level = 1
...
def initAliens():
    ...
    for a in range(18):
        aliens.append(Actor("alien1.png", (210+(a % 6)*80, 100+(int(a/6)*64))))
        aliens[a].status = 0
...
def initBases():
    ...
    for b in range(3):
        for p in range(3):
            bases.append(Actor("base1.png", midbottom = (150+(b*200)+(p*40), 520)))
